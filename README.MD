...
1) Run `./gradlew :kxcel:publishToMavenLocal`
2) Open `~/.m2/repository/dev/shreyash/kxcel/`
...
4) Add these lines to `gradle.properties`:
   ...
   ```
5) Run `./gradlew :kxcel:publishAndReleaseToMavenCentral --no-configuration-cache`
